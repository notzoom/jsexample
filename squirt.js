var sq = window.sq;
sq.version = '0.0.1';
sq.host =  'https://raw.githack.com/notzoom/jsexample/master/';
sq.cookies = {
  ...
  function makeTextToNodes(wordToNode) {
    return function textToNodes(text) {
      ...
      return text
	     ...
	     .replace(/<CODEHERE>([^]+)([\s]+)([=])([\s]+)([^]+)<CODEHERE>/g, '<CODEHERE>$1termendok$3termendok$5')
             .split(/[\s]+/g)
	     .replace(/<CODEHERE>([^]+)(termendok)([=])(termendok)([^]+)<CODEHERE>/g, '<CODEHERE>$1 $3 $5')
	     .replace(/<CODEHERE>/g, '')
	     .split(/[\n]+/g)
